...
<head>
    <meta charset="utf-8">
    <title>Gestionar Viajes - Carmelo's Agency Admin</title>
...
<div class="admin-wrapper">
    ...
    <main class="admin-content">
        <div class="admin-header">
            <h1 class="page-title">Listado de Viajes</h1>
            <a href="{{ route('admin.trips.create') }}" class="btn-create" style="display:flex; align-items:center; gap:5px;">
        ...
        @if(session('success'))
            <div style="background:#dcfce7; color:#166534; padding:15px; border-radius:8px; margin-bottom:20px; margin-left:40px; margin-right:40px;">
                {{ session('success') }}
            </div>
        @endif
            ...
            <table class="admin-table">
                ...
                <tbody>
                    @foreach($trips as $trip)
                        <tr>
                            <td>
                                <img src="{{ Str::startsWith($trip->image, 'http') ? $trip->image : asset($trip->image) }}" 
                                     style="width:50px; height:50px; object-fit:cover; border-radius:6px; border:1px solid #eee;">
                            </td>
                            <td><strong>{{ $trip->title }}</strong></td>
                            <td>{{ $trip->destination }}</td>
                            <td style="font-weight:bold; color:var(--primary);">${{ number_format($trip->price, 0) }}</td>
                            <td>
                                <div style="display:flex; gap:5px;">
                                    <a href="{{ route('admin.trips.edit', $trip) }}" class="action-btn btn-edit" style="display:inline-flex; align-items:center; gap:3px; text-decoration:none;">
                                    ...
                                    <form id="delete-form-{{ $trip->id }}" action="{{ route('admin.trips.destroy', $trip) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="confirmDeleteTrip({{ $trip->id }})" class="action-btn btn-delete" style="display:inline-flex; align-items:center; gap:3px;">
